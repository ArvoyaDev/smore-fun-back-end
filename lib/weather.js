...
class Forecast {
	constructor(date, imgCode, temp, description, stateCode) {
...
const weatherHandler = async (request, response) => {
	...
	if(cache[weather] && !isCacheExpired(cache[weather])){
		...
		try {
			...
			let weatherData = weatherResponse.data.data.map((element) => new Forecast(element.datetime, element.weather.icon, element.temp, element.weather.description, stateCode)); // Pass the state code to the Forecast constructor
