...
router.get('/city-info', async (req, res) => {
	...
	if (!city || !city.trim()) {
		return res.status(400).json({ error: 'City must not be empty or just spaces' });
	}
	...
	try {
		...
		if (locationData[0] && locationData[0].stateCode) {
			npsData = await npsHandler({ query: { stateCode: locationData[0].stateCode, lat: locationData[0].lat, lon: locationData[0].lon } }, res);
